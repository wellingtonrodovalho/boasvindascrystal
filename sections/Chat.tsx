...
const Chat: React.FC = () => {
  const [messages, setMessages] = useState<Message[]>([
    { role: 'model', text: 'Olá! Sou o assistente virtual do Flat Ipê. Como posso ajudar você hoje? Pode me perguntar sobre a senha do Wi-Fi, dicas de restaurantes ou como usar a TV do quarto com os 3 controles.' }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const systemInstruction = `
        ...
        Seja sempre educado, prestativo e utilize um tom acolhedor.
        Responda em Português do Brasil.
        
        INFORMAÇÕES CRÍTICAS SOBRE A TV DO QUARTO:
        O quarto possui uma TV LCD que usa 3 CONTROLES:
        1. Controle da TV: Serve para LIGAR a TV (botão vermelho) e mudar a ENTRADA (Input/Source).
        2. Controle Branco (Intelbras): Serve para mudar os CANAIS ABERTOS e controlar o volume da TV a cabo.
        3. Controle do Mi Stick: Serve para navegar na NETFLIX e YouTube.
        
        Como usar:
        - Para Canais: No controle da TV, mude para Antena/Canais e use o controle Intelbras.
        - Para Netflix: No controle da TV, mude para HDMI e use o controle do Mi Stick.

        OUTRAS INFORMAÇÕES DO FLAT:
        - Wi-Fi: Rede "${WIFI_INFO.network}", Senha "${WIFI_INFO.password}".
        - Check-in: 14h | Check-out: 11h.
        - Localização: Av. Edmundo P. de Abreu, 31, Crystal Place.
        - Fechadura: * + DDD + 5 primeiros dígitos do celular + #.
        - Ar condicionado: Controle na cabeceira.
        - Lixo: Hall dos elevadores à esquerda.
        - Pet: São bem-vindos.
        - Restaurantes: ${LOCAL_PLACES.filter(p => p.category === 'Restaurantes').map(p => p.name).join(', ')}.
        
        Sempre que o hóspede estiver confuso, recomende olhar a seção "Nosso Apartamento" no aplicativo para ver o guia visual.
      `;
